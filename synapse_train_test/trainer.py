...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        writer.add_scalar('info/avg_loss', avg_loss, epoch_num + 1)
        logging.info('epoch %d : avg_loss : %f' % (epoch_num + 1, avg_loss))
        if avg_loss < best_loss:
            best_loss = avg_loss
            best_model_state_dict = model.state_dict()


    torch.save(best_model_state_dict, os.path.join(snapshot_path, 'best_model.pth'))
    logging.info('Best model saved with avg_loss %f' % best_loss)
